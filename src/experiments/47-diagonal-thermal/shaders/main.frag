...
void main() {
  ...
  float c = cos(3.14159 / 4.0);
  float s = sin(3.14159 / 4.0);
  vec2 rotated = vec2(c * centered.x - s * centered.y, s * centered.x + c * centered.y) / vec2(aspect, 1.0) + 0.5;

  vec2 cell = floor(rotated * gridSize);
  vec2 localUv = fract(rotated * gridSize);
  ...
  vec3 color = mix(vec3(0.20, 0.13, 0.10), vec3(0.71, 0.32, 0.18), n);
